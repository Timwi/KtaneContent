...
$(function() {
	...
	function makeTree(tree, parent) {
		try {
			...
			if (!buildwarning) {
				buildwarning = true;
				toastr.warning("An error occurred while generating the page. Some information might be missing from the logfile.", "Page Generation Warning");
			}
		}
	}
	...
	function Bomb(seed) {
		...
		this.ToHTML = function(id) {
			...
			var modinfo = $("<div class='module-info'>").appendTo(info)
			$("<h3>").text("Filtered Log").appendTo(modinfo)
			$("<textarea>").text(this.FilteredLog).appendTo(modinfo)
